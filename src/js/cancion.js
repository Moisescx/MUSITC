...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderPost(i) {
    ...
    document.getElementById("tab-author").textContent =
      p.authorFilter || "Información del autor no disponible.";
    document.getElementById("tab-reflection").textContent =
      p.reflection || p.excerpt || "Reflexión no disponible.";
    ...
    if (p.listenUrl) {
      ...
      a.className =
        "inline-flex items-center gap-2 px-4 py-2 text-white bg-green-600 rounded shadow hover:bg-green-700";
    ...
    if (["author", "reflection", "listen"].includes(currentHash))
      showTab(currentHash);
    else showTab("author");
  }

  function prev() {
    index = (index - 1 + filteredPosts.length) % filteredPosts.length;
    renderPost(index);
  }
  function next() {
    index = (index + 1) % filteredPosts.length;
    renderPost(index);
  }
  ...
  function applyFilters() {
    ...
    filteredPosts = posts.filter((p) => {
      return (
        (!authorValue || p.author === authorValue) &&
  ...
  fetch("./data/post.json")
    ...
    .then((data) => {
      ...
      if (genreParam) filterGenre.value = genreParam;
      if (authorParam) filterAuthor.value = authorParam;

      filteredPosts = [...posts];
      filteredPosts.sort((a, b) => (Number(a.id) || 0) - (Number(b.id) || 0));

      // Si hay parámetros, aplicamos filtros de inmediato
      applyFilters();
